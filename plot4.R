...
par(mfrow = c(2, 2))
plot(data$SetTime, data$Global_active_power, col = columnlines[1], xlab = '', ylab = 'Global Active Power', type = 'l')
plot(data$SetTime, data$Voltage, col = columnlines[1], xlab = 'datetime', ylab = 'Voltage', type = 'l')
plot(data$SetTime, data$Sub_metering_1, col = columnlines[1], xlab = '', ylab = 'Energy sub metering', type = 'l')
lines(data$SetTime, data$Sub_metering_2, col = columnlines[2])
lines(data$SetTime, data$Sub_metering_3, col = columnlines[3])
legend('topright', bty="n", col =columnlines, legend = labels, lwd = 1)
plot(data$SetTime, data$Global_reactive_power,col = columnlines[1], xlab = 'datetime', ylab = 'Global_reactive_power', type = 'l')
